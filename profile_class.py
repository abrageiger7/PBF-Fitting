...
class Profile:
    ...
    def fit_plot(self, zbeta_ind, pbfwidth_ind, gwidth_ind, exp = False, zeta = False, low_pbf = -1, high_pbf = -1):
        ...
        if high_pbf != -1:
            plt.plot(time, fitted_templateh, alpha = 0.35, label = fr'Upper Error; $\tau$ = {round(tau_val_high,0)} $\mu$s', color = 'orange', lw = 3)
        if low_pbf != -1:
            plt.plot(time, fitted_templatel, alpha = 0.35, label = fr'Lower Error; $\tau$ = {round(tau_val_low,0)} $\mu$s', color = 'orange', lw = 3)
        plt.plot(time, fitted_template, label = fr'Best fit; $\tau$ = {round(tau_val,0)} $\mu$s', color = 'red')
        plt.legend()
