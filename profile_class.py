...
class Profile:
    ...
    def chi_plot(self, chi_sq_arr, beta = -1, exp = False, gwidth = -1, pbfwidth = -1, zeta = -1):
        ...
        if gwidth == -1 and pbfwidth == -1:
            ...
            plt.yticks(ticks = np.linspace(0,(294+98),num=10), labels = pbf_ticks)
    ...
    def fit_plot(self, zbeta_ind, pbfwidth_ind, gwidth_ind, exp = False, zeta = False, low_pbf = -1, high_pbf = -1):
        ...
        if high_pbf != -1:
            plt.plot(time, fitted_templateh, alpha = 0.1, label = fr'Upper Error; $\tau$ = {tau_val_high} $\mu$s', color = 'orange')
        if low_pbf != -1:
            plt.plot(time, fitted_templatel, alpha = 0.1, label = fr'Lower Error; $\tau$ = {tau_val_low} $\mu$s', color = 'orange')
        plt.legend()
