...
class Profile:
    ...
    def fit_plot(self, zbeta_ind, pbfwidth_ind, gwidth_ind, exp = False, zeta = False, low_pbf = -1, high_pbf = -1):
        ...
        if high_pbf != -1:
            plt.plot(time, fitted_templateh, alpha = 0.7, label = fr'Upper Error; $\tau$ = {tau_val_high} $\mu$s', color = 'orange')
        if low_pbf != -1:
            plt.plot(time, fitted_templatel, alpha = 0.7, label = fr'Lower Error; $\tau$ = {tau_val_low} $\mu$s', color = 'orange')
        plt.legend()
