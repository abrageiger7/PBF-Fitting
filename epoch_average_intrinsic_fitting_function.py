    ...
    if rerun == 'powerlaws' or rerun == 'both':
        ...
        if amp1_tests == 'default' and amp3_tests == 'default' and cent3_tests == 'default' and width3_tests == 'default':
            ...
            # 3.1 0.01 thin
                ...
                amp1_tests=np.array([1.5])
                amp3_tests=np.array([-1.6])
                cent3_tests=np.array([0.6])
                width3_tests=np.array([-1.3])

            # 3.5 0.01 thin
            elif (beta == '3.5' and (zeta == '0.01') and screen == 'thin'):
                print('Thin with Beta = 3.5 and Zeta = 0.01')
                amp1_tests=np.array([0.8])
                amp3_tests=np.array([-0.8])
                cent3_tests=np.array([0.3])
                width3_tests=np.array([-1.1])
            ...
            # 3.667 0.01 thin
                ...
                amp1_tests=np.array([0.6])
                amp3_tests=np.array([-0.5])
                cent3_tests=np.array([0.3])
                width3_tests=np.array([-1.2])
            ...
            # 3.975 0.01 thin
                ...
                amp1_tests=np.array([0.6])
                amp3_tests=np.array([0.9])
                cent3_tests=np.array([0.1])
                width3_tests=np.array([-0.2])

            # might want to rerun ...
            # 3.667 5.0 thin
            elif (beta == '3.667' and (zeta == '5.0') and screen == 'thin'):
                print('Thin with Beta = 3.667 and Zeta = 5.0')
                amp1_tests=np.array([0.5])
                amp3_tests=np.array([0.5])
                cent3_tests=np.array([0.2])
                width3_tests=np.array([-0.7])
